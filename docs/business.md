...
- **Ambimbola teo aloha** : Solde restant du Sabbat précédent. Ce champ est automatiquement récupéré via l'API lors de l'initialisation de l'application (basé sur la dernière validation de Sabbat enregistrée côté serveur).
...
- **Synchronisation** : Chaque type d'offrande et le lot de dépenses peuvent être envoyés individuellement à l'API. Le bouton de synchronisation est désactivé si le montant correspondant est nul (0 Ar), afin d'éviter l'envoi de données vides.
